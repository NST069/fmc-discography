...
const Discography = ({loading, loadingPage, albums, currentAlbum, labels, getAll, getAllbyLabel, getAlbumById, sortPosts, sortingOrder, setSortingOrder})=>{
    ...
    return(<>
        <div className="flex flex-col mx-2 md:flex-row justify-around mt-5">
            <div className="bg-gray-900 text-sm text-gray-400 leading-none border-2 border-gray-800 rounded-full inline-flex justify-between">
                ...
                {labels.map(label=>
                    <button key={label.name} className={`${filterLabel===label.value?"active":""} inline-flex items-center transition-colors duration-300 ease-in focus:outline-none hover:text-gray-300 focus:text-gray-300 rounded-full px-4 py-2`} 
                        id={label.name}
                        onClick={(event)=>{
                            event.preventDefault();
            ...
            <div className="bg-gray-900 text-sm text-gray-400 leading-none border-2 border-gray-800 rounded-full inline-flex justify-between">
                {[{name: "New", value: "1"}, {name: "Artist", value:"2"}, {name: "Title", value:"3"}].map((rule, idx)=>
                    <button key={rule.name} className={`${selectedRule===rule.value?"active":""} inline-flex items-center transition-colors duration-300 ease-in focus:outline-none hover:text-gray-300 focus:text-gray-300 rounded-full px-4 py-2`} 
                        id={rule.name}
                        onClick={(event)=>{
                            event.preventDefault();
