  ...
  const scramble_text = (text) => {
    if (!complete_map) {
      ...
      for (var key in reversal_map) {
        ...
        if (!reversal_map[val]) complete_map[reversal_map[key]] = key;
    ...
    <div className="flex mx-auto items-center justify-center fixed top-0 right-0 h-screen w-screen">
      <form className="w-full max-w-xl bg-gray-900 rounded-lg px-4 pt-2">
        <div className="flex flex-wrap -mx-3 mb-6">
          <h2 className="px-4 pt-3 pb-2 text-gray-400 text-lg">
            ＦＵＬＬＷＩＤＴＨ Text Generator
          </h2>
          ...
          <div className="w-full flex items-start md:w-full px-3">
            <div className="-mr-1">
              <input
                type="submit"
                className="bg-gray-900 text-gray-300 font-medium py-1 px-4 border border-gray-600 rounded-lg tracking-wide mr-1 hover:bg-gray-900"
                value="Ｆｕｌｌｗｉｄｔｈ'ify"
                onClick={(event) => {
